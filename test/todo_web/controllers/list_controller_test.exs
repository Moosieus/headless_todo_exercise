defmodule TodoWeb.ListControllerTest do
  ...
  defp create_tasks(%{conn: %Plug.Conn{} = conn, list: list}) do
  ...
  describe "create task" do
    ...
    test "renders parent list when data is valid", %{conn: conn, list: list} do
      task_attrs = %{list_id: list.id, text: "a new task 410"}
      conn = post(conn, ~p"/api/lists/tasks", task: task_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, ~p"/api/lists/#{list.id}")

      # pick up here, find the new task in the list response.
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/api/lists/tasks", list: @invalid_task_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
